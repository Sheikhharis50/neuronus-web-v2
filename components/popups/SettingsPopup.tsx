...
export const SettingsPopup = ({ isOpen, onClose }: Props) => {
  ...
  const handleVerifyAndEnable = async () => {
    if (!otpToken) { toast.error("Please enter the OTP code"); return; }
    setIsLoading(true);
    try {
      await authService.manageTotp(true, otpToken);
      if (typeof localStorage !== "undefined") localStorage.setItem("is_2fa", "true");
  ...
  const handleDisable2fa = async () => {
    ...
    try {
      ...
      if (typeof localStorage !== "undefined") localStorage.setItem("is_2fa", "false");
          ...
          return (
            ...
            >
              <div className="w-9 h-9 flex items-center justify-center group-hover:scale-110 transition-transform duration-200">
                <Image src={tool.icon} alt={tool.name} width={28} height={28} className="w-auto h-auto" />
              </div>
          ...
          <div className="flex flex-col items-center">
            ...
            {!isDisablingMode && totpData?.qr_code && (
              <div className="bg-white p-4 border border-gray-200 rounded-xl mb-6 shadow-sm">
                <Image src={totpData.qr_code} alt="QR Code" width={200} height={200} />
              </div>
            )}
    ...
    >
      ...
      <div className="w-full md:w-[280px] bg-[#F4F7FC] p-6 lg:p-8 flex flex-col border-r border-gray-100/50">
        ...
        <div className="flex flex-col space-y-3 grow">
          ...
          <SidebarButton tab="server"        icon={<ServerIcon />} label="Server Location" />
          {/* Tools — clicking this switches right side to tools grid */}
          <SidebarButton tab="tools"         icon={<ToolsIcon />}  label="Tools" />
          <SidebarButton tab="subscriptions" icon={<BoxIcon />}    label="Subscriptions" />
      ...
      <div className="flex-1 bg-[#F9FBFF] p-6 md:p-10 flex flex-col">
        ...
        {activeTab === "server"        && <PlaceholderContent title="Server Location" />}
        {activeTab === "subscriptions" && <PlaceholderContent title="Subscriptions" />}
      </div>
    </Modal>
  );
};